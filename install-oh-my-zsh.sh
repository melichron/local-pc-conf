...
Usage:
  ...
  install-oh-my-zsh.sh --install-dest [home|skel|both]
  install-oh-my-zsh.sh --to-skel

Environment:
  INSTALL_TARGET=zsh|tmux|screen|bash-it|all
  INSTALL_DEST=home|skel|both
...
parse_args() {
    ...
    while [[ $# -gt 0 ]]; do
        ...
        -d | --install-dest)
            shift
            [[ $# -gt 0 ]] || die "--install-dest requires a value: home|skel|both"
            [[ -z "$cli_dest" ]] || die "Install destination already specified: $cli_dest"
            cli_dest="$1"
            shift
            ;;
        --to-skel)
        ...
        -h | --help)
            ...
            ;;
        *)
            die "Unknown argument: $1 (expected targets, --target, --install-dest, --to-skel, --help)"
            ;;
        esac
    done
    ...
    *)
    ...
    home | skel | both) ;;
    *)
        die "Unsupported install destination: ${INSTALL_DEST}. Use home, skel, or both."
        ;;
    esac
}

dest_includes_home() {
    [[ "$INSTALL_DEST" == "home" || "$INSTALL_DEST" == "both" ]]
}

dest_includes_skel() {
    [[ "$INSTALL_DEST" == "skel" || "$INSTALL_DEST" == "both" ]]
}

build_install_homes() {
    INSTALL_HOMES=()
    if dest_includes_home; then
        INSTALL_HOMES+=("$HOME")
    fi
    if dest_includes_skel; then
        INSTALL_HOMES+=("/etc/skel")
    fi
}
...
write_p10k() {
    cat >"$INSTALL_HOME/.p10k.zsh" <<'P10K_EOF'
# Generated by Powerlevel10k configuration wizard on 2021-02-21 at 11:37 EET.
...
P10K_EOF
    chmod 0644 "$INSTALL_HOME/.p10k.zsh"
}

write_zshrc() {
    cat >"$INSTALL_HOME/.zshrc" <<'ZSHRC_EOF'
...
#plugins=(git)
plugins=(git docker terraform helm kubectl vscode colorize docker-compose sudo aws history zsh-autosuggestions ubuntu singlechar safe-paste)
...
ZSHRC_EOF
    chmod 0664 "$INSTALL_HOME/.zshrc"
}

write_tmuxrc() {
    cat >"$INSTALL_HOME/.tmux.conf" <<'TMUXRC_EOF'
##### GENERAL BEHAVIOR #####
...
TMUXRC_EOF
    chmod 0644 "$INSTALL_HOME/.tmux.conf"
}
...
write_screenrc() {
    ...
    if [[ -f "${bundled_screenrc}" ]]; then
        cp "${bundled_screenrc}" "$INSTALL_HOME/.screenrc"
        chmod 0644 "$INSTALL_HOME/.screenrc"
        return
    fi

    cat >"$INSTALL_HOME/.screenrc" <<'SCREENRC_EOF'
...
ensure_bash_it_block_in_bashrc() {
    local bashrc="$INSTALL_HOME/.bashrc"
...
setup_zsh() {
    ...
    if [[ ! -d "$zsh_dir" ]]; then
        ...
        git clone -c core.autocrlf=input --depth=1 https://github.com/ohmyzsh/ohmyzsh "$zsh_dir"
    else
        log "oh-my-zsh already present"
    fi

    log "Ensuring custom dirs"
    mkdir -p "$zsh_dir/completions" "$zsh_dir/functions"
    chmod 0755 "$zsh_dir/completions" "$zsh_dir/functions"
    ...
    ensure_git_repo "https://github.com/zsh-users/zsh-autosuggestions" \
        "$zsh_dir/custom/plugins/zsh-autosuggestions"

    backup_file "$INSTALL_HOME/.p10k.zsh"
    write_p10k

    backup_file "$INSTALL_HOME/.zshrc"
    write_zshrc
}
...
setup_screen() {
    backup_file "$INSTALL_HOME/.screenrc"
    write_screenrc
}

setup_bash_it() {
    local bash_it_dir="$INSTALL_HOME/.bash_it"
    ...
    backup_file "$INSTALL_HOME/.bashrc"
    ensure_bash_it_block_in_bashrc
}
...
main() {
    ...
    for INSTALL_HOME in "${INSTALL_HOMES[@]}"; do
        log "Applying setup to ${INSTALL_HOME}"
        if target_includes_zsh; then
            setup_zsh
        fi
        if target_includes_tmux; then
    ...
    log "Done."
    if dest_includes_home && target_includes_zsh; then
        log "Open a new terminal or run: exec zsh"
        log "Optional: set zsh as default shell: chsh -s \"$(command -v zsh)\""
    fi
    if dest_includes_home && target_includes_tmux; then
        log "Run tmux with: tmux"
    fi
    if dest_includes_home && target_includes_screen; then
        log "Run screen with: screen"
    fi
    if dest_includes_home && target_includes_bash_it; then
        log "Open a new shell or run: source ~/.bashrc"
    fi
    if dest_includes_skel; then
        log "Installed templates to /etc/skel for newly created users."
    fi
}
